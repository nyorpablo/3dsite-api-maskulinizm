...
class ParserController extends Controller
{
    ...
    public function stl_parser(Request $request){
        $getToken = UserApiToken::where('user_api_key', $request['api_key'])->with('user')->get();
        ...
        $user_subscription = UserApiSubscription::where('user_id', $getToken[0]->user->id)->get();

        $update_subsctiption_usability = UserApiSubscription::where('user_id', $user_subscription[0]->id)->update([
                                                                'usage' => $user_subscription[0]->usage - 1,
                                                            ]);
